...
#############Defined Strings#############
addon_id     = 'plugin.video.smokinbarrel'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.smokinbarrel/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.smokinbarrel/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to Smoking Barrel" 
			line3 = ('[COLOR red]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('Smoking Barrel', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
		...
		auth = tools.OPEN_URL(auth)
		if not auth=="":
			tools.addDir('[COLOR grey]Account Information[/COLOR]','url',6,icon,fanart,'')
			tools.addDir('[COLOR grey]Live TV[/COLOR]','live',1,icon,fanart,'')
			tools.addDir('[COLOR grey]Catchup TV[/COLOR]','url',12,icon,fanart,'')
			if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)') or xbmc.getCondVisibility('System.HasAddon(script.ivueguide)'):
				tools.addDir('[COLOR grey]TV Guide[/COLOR]','pvr',7,icon,fanart,'')
			tools.addDir('[COLOR grey]VOD[/COLOR]','vod',3,icon,fanart,'')
			tools.addDir('[COLOR grey]Search[/COLOR]','url',5,icon,fanart,'')
			tools.addDir('[COLOR grey]Settings[/COLOR]','url',8,icon,fanart,'')
			tools.addDir('[COLOR grey]Extras[/COLOR]','url',16,icon,fanart,'')
def home():
...
##########################################
def catchup():
    loginurl   = "http://allviewtv.uk:80/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("Smoking Barrel", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search Smoking Barrel:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('Smoking Barrel', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('Smoking Barrel', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('Smoking Barrel', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('Smoking Barrel', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('Smoking Barrel', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.smokinbarrel/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('Smoking Barrel', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.smokinbarrel')
	...
	loginurl   = "http://allviewtv.uk:80/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://allviewtv.uk:80/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('Smoking Barrel','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
