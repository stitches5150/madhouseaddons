...
addon_name = 'bonecrusher'
addon_icon = xbmcaddon.Addon().getAddonInfo('icon')
addon_path = xbmc.translatePath(('special://home/addons/plugin.video.bonecrusher')).decode('utf-8')
module_path = xbmc.translatePath(('special://home/addons/script.module.bonecrusher')).decode('utf-8')


def main():
    fum_ver = xbmcaddon.Addon(id='script.module.bonecrusher').getAddonInfo('version')
    updated = xbmcaddon.Addon(id='plugin.video.bonecrusher').getSetting('module_base')
    ...
    log('bonecrusher Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
        log('bonecrusher Service - Exception: \n %s' % (str(failure)), LOGINFO)
        return
    ...
    xbmcaddon.Addon(id='plugin.video.bonecrusher').setSetting('module_base', fum_ver)
...
DEBUGPREFIX = '[COLOR red][ bonecrusher DEBUG ][/COLOR]'
