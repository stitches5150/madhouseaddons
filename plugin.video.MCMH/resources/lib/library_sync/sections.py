...
class Section(object):
    ...
    def from_plex_db(self, section_id, mchmdb=None):
        """
        Reads section with id section_id from the mchm.db
        """
        if mchmdb:
            section = mchmdb.section(section_id)
        else:
    ...
    def to_plex_db(self, mchmdb=None):
        """
        Writes this Section to the mchm.db, potentially overwriting
        (INSERT OR REPLACE)
        """
        if not self:
            raise RuntimeError('Section not clearly defined: %s' % self)
        if mchmdb:
            mchmdb.add_section(self.section_id,
                               self.name,
                               self.section_type,
                               self.kodi_tagid,
                               self.sync_to_kodi,
    ...
    def addon_path(self, args):
        """
        Returns the plugin path pointing back to MHC for key in order to browse
    ...
    def _build_node(self, node_type, node_name, args, content, pms_node):
        ...
        if pms_node:
            ...
            # to point back to plugin://plugin.video.MCMH
    ...
    def remove_from_plex(self, mchmdb=None):
        """
        Removes this sections completely from the Plex DB
        """
        if mchmdb:
            mchmdb.remove_section(self.section_id)
        else:
            with PlexDB(lock=False) as mchmdb:
                mchmdb.remove_section(self.section_id)
...
def force_full_sync():
    ...
    full sync (not delta)
    """
    LOG.info('Telling MHC to do a full sync instead of a delta sync')
    with PlexDB() as mchmdb:
        mchmdb.force_full_sync()


def _save_sections_to_plex_db(sections):
    with PlexDB() as mchmdb:
        for section in sections:
            section.to_plex_db(mchmdb=mchmdb)


def _retrieve_old_settings(sections, old_sections):
    """
    Overwrites the MHC settings for sections, grabing them from old_sections
...
def _delete_kodi_db_items(section):
    ...
    for plex_type, context in types:
        while True:
            with PlexDB() as mchmdb:
                plex_ids = list(mchmdb.plexid_by_sectionid(section.section_id,
                                                           plex_type,
                                                           BATCH_SIZE))
                with kodi_context(texture_db=True) as kodidb:
                    typus = context(None, mchmdb=mchmdb, kodidb=kodidb)
...
def _sync_from_pms(pick_libraries):
    ...
    with PlexDB() as mchmdb:
        for section_db in mchmdb.all_sections():
...
def delete_videonode_files():
    """
    Removes all the MHC video node files under userdata/library/video that
    start with 'Plex-'
    """
