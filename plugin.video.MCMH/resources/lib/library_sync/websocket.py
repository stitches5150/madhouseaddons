...
def store_activity_message(data):
    ...
    WATCH OUT for this if it's triggered by our MHC library scan!
    """
    global WEBSOCKET_MESSAGES
    for message in data:
        ...
        with PlexDB(lock=False) as mchmdb:
            typus = mchmdb.item_by_id(plex_id, plex_type=None)
        if not typus:
...
def process_playing(data):
    ...
    for message in data:
        ...
        if utils.settings('plex_serverowned') != 'false':
            # Identify the user - same one as signed on with MHC? Skip
            # update if neither session's username nor userid match
            # (Owner sometime's returns id '1', not always)
            if not app.ACCOUNT.plex_token and session['userId'] == '1':
                # MHC not signed in to plex.tv. Plus owner of PMS is
                # playing (the '1').
...
def cache_artwork(plex_id, plex_type, kodi_id=None, kodi_type=None):
    """
    Triggers caching of artwork (if so enabled in the MHC settings)
    """
    if not CACHING_ENALBED:
        return
    if not kodi_id:
        with PlexDB(lock=False) as mchmdb:
            item = mchmdb.item_by_id(plex_id, plex_type)
        if not item:
