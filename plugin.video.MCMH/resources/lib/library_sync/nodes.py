...
ICON_PATH = 'special://home/addons/plugin.video.MCMH/icon.png'
RECOMMENDED_SCORE_LOWER_BOUND = 7

# Logic of the following nodes:
# (node_type,
#  label/node name,
#  args for MHC add-on callback,
#  Kodi "content",
#  Bool: does this node's xml even point back to MHC add-on callback?
#  )
NODE_TYPES = {
    v.PLEX_TYPE_MOVIE: (
        ('plex_ondeck',
         ...
         True),
        ('ondeck',
         utils.lang(39502),  # "MHC On Deck (faster)"
         {},
         v.CONTENT_TYPE_MOVIE,
         False),
        ('recent',
...
def node_pms(section, node_name, args):
    ...
    xml that filters and sorts, but point to MHC add-on path
...
def node_recent(section, node_name):
    ...
    if ((section.section_type == v.PLEX_TYPE_SHOW and
        ...
        # Adds an additional rule if user deactivated the MHC setting
