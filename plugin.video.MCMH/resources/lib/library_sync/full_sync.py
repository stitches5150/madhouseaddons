...
class FullSync(common.fullsync_mixin):
    ...
    def process_item(self, xml_item):
        ...
        if not self.repair and self.mchmdb.checksum(plex_id, self.plex_type) == \
                int('%s%s' % (plex_id,
            ...
            while True:
                with section.context(self.current_sync) as itemtype:
                    for i, (last, xml_item) in enumerate(loop):
                        ...
                        itemtype.mchmdb.update_last_sync(int(xml_item.attrib['ratingKey']),
                                                         section.plex_type,
                                                         self.current_sync)
                        self.current += 1
                        self.update_progressbar()
        ...
        while True:
            ...
            if self.section_success:
                ...
                with PlexDB() as mchmdb:
                    ...
                    mchmdb.update_section_last_sync(section.section_id,
                                                    self.current_sync)
        ...
        for plex_type, context in kinds:
            ...
            while True:
                with context(self.current_sync) as ctx:
                    plex_ids = list(ctx.mchmdb.plex_id_by_last_sync(plex_type,
                                                                    self.current_sync,
                                                                    BATCH_SIZE))
