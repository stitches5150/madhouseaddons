...
class SubscriptionMgr(object):
    ...
    def _timeline_dict(self, player, ptype):
        with app.APP.lock_playqueues:
            ...
            # Get the plex id from the MHC playqueue not info, as Kodi jumps to
    ...
    def signal_stop(self):
        """
        Externally called on MHC shutdown to ensure that MHC signals a stop to
        the PMS. Otherwise, MHC might be stuck at "currently playing"
        """
    ...
    def notify(self):
        """
        Causes MHC to tell the PMS and Plex Companion players to receive a
        ...
        with app.APP.lock_subscriber:
            ...
            # Update the MHC info with what's playing on the Kodi side
            for player in players.values():
                update_player_info(player['playerid'])
            # Check whether we can use the CURRENT info or whether MHC is still
            # initializing
            if self._playqueue_init_done(players) is False:
                LOG.debug('MHC playqueue is still initializing - skip update')
                return
    ...
    def add_subscriber(self, protocol, host, port, uuid, command_id):
        """
        Adds a new Plex Companion subscriber to MHC.
        """
