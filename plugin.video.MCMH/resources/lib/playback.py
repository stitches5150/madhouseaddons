...
def _playback_triage(plex_id, plex_type, path, resolve):
    ...
    with app.APP.lock_playqueues:
        ...
        try:
            ...
            LOG.debug('MHC playqueue yet empty, need to initialize playback')
            initiate = True
        else:
...
def _playback_init(plex_id, plex_type, playqueue, pos):
    ...
    """
    LOG.info('Initializing MHC playback')
    ...
    # By design, MHC will start Kodi playback using Player().play(). Kodi
    ...
    thread.start()
    # Ensure that MHC playqueue monitor ignores the changes we just made
    ...
    for i, item in enumerate(xml):
        ...
        if (app.PLAYSTATE.context_menu_play is False and
            ...
            with PlexDB(lock=False) as mchmdb:
                db_item = mchmdb.item_by_id(api.plex_id, api.plex_type)
        ...
        for part, _ in enumerate(item[0]):
            api.part = part
            if kodi_id is None:
                # Need to redirect again to MHC to conclude playback
                ...
                # path = path.replace('plugin.video.MCMH.tvshows',
                #                     'plugin.video.MCMH', 1)
                # path = path.replace('plugin.video.MCMH.movies',
                #                     'plugin.video.MCMH', 1)
...
def _process_stack(playqueue, stack):
    """
    Takes our stack and adds the items to the MHC and Kodi playqueues.
    """
...
def _set_resume(listitem, item, api):
    ...
    if item.resume is True:
        ...
        with PlexDB(lock=False) as mchmdb:
            db_item = mchmdb.item_by_id(item.plex_id, item.plex_type)
        if db_item:
...
def _conclude_playback(playqueue, pos):
    ...
    ONLY if actually being played (e.g. at 5th position of a playqueue).
        ...
        Init Kodi Playback (depending on situation):
            start playback
            return MHC listitem attached to result
    """
    ...
    if resolve is True:
        ...
        thread = Thread(target=app.APP.player.play,
                        args={'item': utils.try_encode(playurl),
                              'listitem': listitem})
        thread.setDaemon(True)
        LOG.info('Done initializing MHC playback, starting Kodi player')
        thread.start()
