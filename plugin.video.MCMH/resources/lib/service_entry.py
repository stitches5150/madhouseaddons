...
WINDOW_PROPERTIES = (
    ...
    "plex_allows_mediaDeletion", "madhouseconnect.command", "plex_result")
...
class Service(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            # "The current Kodi version is not supported by MHC. Please consult
            # the Plex forum."
        ...
        # Reset some status in the MHC settings
        # toggled to "No"
    ...
    def reset_playlists_and_nodes(self):
        """
        Resets the Kodi playlists and nodes for all the MHC libraries by
    ...
    def ServiceEntryPoint(self):
        ...
        # Main MHC program loop
        while not self.isCanceled():

            # Check for MHC commands from other Python instances
            plex_command = utils.window('madhouseconnect.command')
            if plex_command:
                # Commands/user interaction received from other MHC Python
                # instances (default.py and context.py instead of service.py)
                utils.window('madhouseconnect.command', clear=True)
                task = None
                ...
                elif plex_command == 'RESET-MHC':
                    utils.reset()
                elif plex_command == 'EXIT-MHC':
        ...
        app.APP.stop_pkc = True
        # Load/Reset MHC entirely - important for user/Kodi profile switch
...
def start():
    # Safety net - Kody starts MHC twice upon first installation!
    if utils.window('plex_service_started') == 'true':
        LOG.info('Another service.py instance is already running - shutting '
                 'it down now')
        # Telling the other Python instance of MHC to shut down now
        i = 0
        while utils.window('madhouseconnect.command'):
            xbmc.sleep(20)
            i += 1
            if i > 300:
                LOG.error('Could not tell other MHC instance to shut down')
                return
        utils.window('madhouseconnect.command', value='EXIT-MHC')
        ...
        while utils.window('plex_service_started'):
            ...
            i += 1
            if i > 300:
                LOG.error('Could not shut down other MHC instance')
                return
    ...
    if DELAY and xbmc.Monitor().waitForAbort(DELAY):
        # Start the service
        LOG.info("Abort requested while waiting. MHC not started.")
    else:
