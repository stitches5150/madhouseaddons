...
# -*- coding: utf-8 -*-
"""
Various functions and decorators for MHC
"""
...
ADDON = xbmcaddon.Addon(id='plugin.video.MCMH')
...
def setGlobalProperty(key, val):
    xbmcgui.Window(10000).setProperty(
        'plugin.video.MCMH.{0}'.format(key), val)


def setGlobalBoolProperty(key, boolean):
    xbmcgui.Window(10000).setProperty(
        'plugin.video.MCMH.{0}'.format(key), boolean and '1' or '')


def getGlobalProperty(key):
    return xbmc.getInfoLabel(
        'Window(10000).Property(plugin.video.MCMH.{0})'.format(key))
...
def lang(stringid):
    """
    Central string retrieval from strings.po. If not found within MHC,
...
def dialog(typus, *args, **kwargs):
    ...
    if 'icon' in kwargs:
        types = {
            '{plex}': 'special://home/addons/plugin.video.MCMH/icon.png',
...
def wipe_synched_playlists():
    ...
    from . import mchm_db
    try:
        with mchm_db.PlexDB() as mchmdb:
            mchmdb.cursor.execute('SELECT kodi_path FROM playlists')
            playlist_paths = [x[0] for x in mchmdb.cursor]
    except OperationalError:
...
def wipe_database(reboot=True):
    ...
    try:
        with mchm_db.PlexDB() as mchmdb:
            if mchmdb.songs_have_been_synced():
...
def reset(ask_user=True):
    """
    User navigated to the MHC settings, Advanced, and wants to reset the Kodi
    database and possibly MHC entirely
    """
...
class XmlKodiSetting(object):
    ...
    def __enter__(self):
        try:
            ...
            # "Kodi cannot parse {0}. MHC will not function correctly. Please
        ...
        if self.write_xml is True:
            ...
            try:
                ...
                LOG.error('Could not save xml %s. Error: %s',
                          self.filename, err)
                # Could not change the Kodi settings file {0}. MHC might not
    ...
    def _remove_empty_elements(self):
        """
        Deletes all empty XML elements, otherwise Kodi/MHC gets confused
