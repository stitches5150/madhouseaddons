...
def check_migration():
    ...
    # Ensure later migration if user downgraded MHC!
    utils.settings('last_migrated_PKC_version', value=v.ADDON_VERSION)

    if last_migration == '':
        LOG.info('New, clean MHC installation - no migration necessary')
        return
    elif last_migration == v.ADDON_VERSION:
        LOG.info('Already migrated to MHC version %s' % v.ADDON_VERSION)
        return

    if not utils.compare_version(last_migration, '1.8.2'):
        LOG.info('Migrating to version 1.8.1')
        # Set the new MHC theMovieDB key
    ...
    if not utils.compare_version(last_migration, '2.8.7'):
        ...
        with PlexDB() as mchmdb:
            mchmdb.cursor.execute('DROP INDEX IF EXISTS ix_playlists_3')
            # Index will be automatically recreated on next MHC startup
