...
# Minimal MHC version needed for the Kodi database - otherwise need to recreate
...
DB_PLEX_PATH = try_decode(xbmc.translatePath("special://database/mchm.db"))
...
ADDON_TYPE = {
    PLEX_TYPE_MOVIE: 'plugin.video.MCMH.movies',
    PLEX_TYPE_CLIP: 'plugin.video.MCMH.movies',
    PLEX_TYPE_EPISODE: 'plugin.video.MCMH.tvshows',
    PLEX_TYPE_SEASON: 'plugin.video.MCMH.tvshows',
    PLEX_TYPE_SHOW: 'plugin.video.MCMH.tvshows',
    PLEX_TYPE_SONG: 'plugin.video.MCMH',
    PLEX_TYPE_ALBUM: 'plugin.video.MCMH',
    PLEX_TYPE_ARTIST: 'plugin.video.MCMH',
    PLEX_TYPE_PLAYLIST: 'plugin.video.MCMH'
}
...
def database_paths():
    '''
    Set the Kodi database paths - MHC will choose the HIGHEST available and
    ...
    if KODIVERSION not in (17, 18, 19):
        raise RuntimeError('Kodiversion %s not supported by MHC' % KODIVERSION)
    ...
    if (DB_VIDEO_VERSION is None or
        ...
        raise RuntimeError('Kodi database versions not supported by MHC')
