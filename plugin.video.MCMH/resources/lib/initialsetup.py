...
class InitialSetup(object):
    ...
    Will ask the user initial questions on first MHC boot
    """
    def __init__(self):
    ...
    def write_credentials_to_settings(self):
        """
        Writes Plex username, token to plex.tv and Plex id to MHC settings
        """
    ...
    def _user_pick_pms(self):
        ...
        while True:
            if https_updated is False:
                ...
                # Close the MHC info "Searching for PMS"
                executebuiltin("Dialog.Close(all, true)")
    ...
    def _add_sources(root, extension):
        ...
        for source in root.findall('.//path'):
            ...
            for _ in range(0, count):
                ...
                etree.SubElement(
                    source,
                    'name').text = "Madhouse Connect Masterlock Hack"
                etree.SubElement(
                    source,
                    'path',
    ...
    def setup(self):
        ...
        try:
            with utils.XmlKodiSetting('advancedsettings.xml',
                ...
                if utils.settings('useDirectPaths') != '1':
                    # Disable cleaning of library - not compatible with MHC
        ...
        # Hack to make MHC Kodi master lock compatible
        try:
        ...
        # breaking playback reporting for MHC
        warn = False
        ...
        if warn:
            ...
            if utils.settings('warned_setting_videoplayer.autoplaynextitem') == 'false':
                ...
                # enabled. This could break MHC. Deactivate?
        ...
        if app.CONN.server:
            ...
            if self.check_existing_pms():
                ...
                if utils.settings('kodi_db_has_been_wiped_clean') == 'false':
                    # If the user chose to go to the MHC settings on the first run
        ...
        # Use Add-on Paths (default, easy) or Direct Paths? MHC will not work
        ...
        if goto_settings:
            LOG.info('User chose to go to the MHC settings - suspending MHC')
            app.APP.stop_pkc = True
            executebuiltin(
                'Addon.OpenSettings(plugin.video.MCMH)')
            return
        utils.reboot_kodi()
