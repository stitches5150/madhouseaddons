...
def initialize():
        """
        Run once upon MHC startup to verify that plex db exists.
        """
        with PlexDBBase() as mchmdb:
            mchmdb.cursor.execute('''
...
def wipe(table=None):
    ...
    with PlexDBBase() as mchmdb:
        if table:
            ...
            mchmdb.cursor.execute("SELECT name FROM sqlite_master WHERE type = 'table'")
            tables = [i[0] for i in mchmdb.cursor.fetchall()]
        for table in tables:
            mchmdb.cursor.execute('DROP table IF EXISTS %s' % table)
