...
class HostedLink:
    def __init__(self, title, year, imdb=None, tvdb=None, host=None, include_disabled=False, timeout=30, exclude=None, enable_debrid=False):
        ...
        xbmcvfs.mkdir(xbmc.translatePath(xbmcaddon.Addon("script.module.odin").getAddonInfo('profile')))
        self.cache_location = os.path.join(
            xbmc.translatePath(xbmcaddon.Addon("script.module.odin").getAddonInfo('profile')).decode('utf-8'),
            'url_cache.db')
    ...
    def get_url(scraper, title, show_year, year, season, episode, imdb, tvdb, type, cache_location, maximum_age, check_url = False, debrid = False):
        cache_enabled = xbmcaddon.Addon('script.module.odin').getSetting("cache_enabled") == 'true'
        try:
