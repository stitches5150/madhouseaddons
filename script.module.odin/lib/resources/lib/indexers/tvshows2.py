...
class tvshows:
    ...
    def search(self):
        try:
            ...
            url = '%s?action=tvshows2&url=%s' % (sys.argv[0], urllib.quote_plus(url))
            control.execute('Container.Update(%s)' % url)
        except:
            return
    def years(self):
    ...
    # def genres(self):
        ...
        # for i in genres: self.list.append({'name': cleangenre.lang(i[0], self.lang), 'url': self.genre_link % i[1], 'image': 'icon.png', 'action': 'tvshows2'})
        # self.addDirectory(self.list)
        # return self.list
    def genres(self):
        try:
            ...
            for i in range(0, len(self.list)): self.list[i].update({'image': 'icon.png', 'action': 'tvshows2'})
            self.addDirectory(self.list)
            return self.list
        except:
            return
    ...
    def networks(self):
        ...
        for i in networks: self.list.append({'name': i[0], 'url': self.network_link % (i[1]), 'image': 'icon.png', 'action': 'tvshows2'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i), 'url': self.certification_link % str(i).replace('-', '_').lower(), 'image': 'icon.png', 'action': 'tvshows2'})
        self.addDirectory(self.list)
        return self.list
    ...
    def persons(self, url):
        ...
        for i in range(0, len(self.list)): self.list[i].update({'action': 'tvshows2'})
        self.addDirectory(self.list)
        return self.list
    ...
    def userlists(self):
        ...
        self.list = userlists
        for i in range(0, len(self.list)): self.list[i].update({'image': 'userlists.png', 'action': 'tvshows2'})
        self.addDirectory(self.list)
        return self.list
    ...
    def favourites(self):
        try:
            items = favourites.getFavourites('tvshows2')
    ...
    def tvshowDirectory(self, items):
        ...
        indicators = playcount.getTVShowIndicators(refresh=True) if action == 'tvshows2' else playcount.getTVShowIndicators()
        ...
        for i in items:
            try:
                ...
                if not action == 'tvFavourites':cm.append(('Add to Watchlist', 'RunPlugin(%s?action=addFavourite&meta=%s&content=tvshows2)' % (sysaddon, sysmeta)))
                if action == 'tvFavourites': cm.append(('Remove From Watchlist', 'RunPlugin(%s?action=deleteFavourite&meta=%s&content=tvshows2)' % (sysaddon, sysmeta)))
        ...
        control.content(syshandle, 'tvshows2')
        # control.do_block_check(False)
        control.directory(syshandle, cacheToDisc=True)
        views.setView('tvshows2', {'skin.confluence': 500})
